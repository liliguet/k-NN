function accur = knnclassifier_DMD(traindata, testdata, K)
%KNN Classifier function
    ...
    for i = 1 : size(testdata)
        x = testdata(i,1:w-1);             
        for d = 1:size(traindata, 1) 
            denominator = 1 + min(traindata(d,1:w - 1),x); 
            numerator = max(traindata(d,1:w - 1),x) - min(traindata(d,1:w - 1),x);  
